...
router.get('/support', async (req: Request, res: Response) => {
    ...
    try {
        ...
        const [accRows]: any = await pool.execute(
            'SELECT DiscordAvatar FROM accounts WHERE ID = ?', [userId]
        );
        const user = {
            ...session.user,
...
router.post('/support/create', async (req: Request, res: Response) => {
    try {
        ...
        const [existing]: any = await pool.execute(
            `SELECT id FROM support_tickets WHERE account_id = ? AND status != 'closed' LIMIT 1`,
            [userId]
        );
        ...
        const [result]: any = await pool.execute(
            `INSERT INTO support_tickets (account_id, username, subject) VALUES (?, ?, ?)`,
            [userId, username, subject.trim()]
        );
        const ticketId = result.insertId;

        // Add first message
        await pool.execute(
            `INSERT INTO support_messages (ticket_id, sender_id, sender_name, is_admin, message)
             VALUES (?, ?, ?, 0, ?)`,
...
router.get('/support/:id', async (req: Request, res: Response) => {
    try {
        ...
        if (isNaN(ticketId)) return res.redirect('/support');

        const userId    = session.user.id;
        const adminUser = await isAdmin(userId);

        // Fetch ticket
        const [ticketRows]: any = await pool.execute(
            'SELECT * FROM support_tickets WHERE id = ?', [ticketId]
        );
        if (!ticketRows.length) return res.redirect('/support');

        const ticket = ticketRows[0];

        // Access control: user can only view their own ticket
        if (!adminUser && ticket.account_id !== userId) return res.redirect('/support');
...
router.post('/support/:id/reply', async (req: Request, res: Response) => {
    try {
        ...
        const ticketId = parseInt(req.params.id);
        if (isNaN(ticketId)) return res.status(400).json({ error: 'Invalid ticket' });
        ...
        await pool.execute(
            `INSERT INTO support_messages (ticket_id, sender_id, sender_name, is_admin, message)
             VALUES (?, ?, ?, ?, ?)`,
            [ticketId, userId, username, adminUser ? 1 : 0, message.trim()]
        );
...
router.post('/support/:id/close', async (req: Request, res: Response) => {
    try {
        const session = req.session as any;
        if (!session?.user) return res.status(401).json({ error: 'Not logged in' });
